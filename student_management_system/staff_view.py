    ...
    if user.user_type != '2' or user.staff.current_position != 'teacher':
        ...
        if request.method != 'POST':
            ...
            try:
                ...
                return redirect(reverse('add_result_list'))
            except:
...
class StaffEditResult(View):
    ...
    def post(self, request,  *args, **kwargs):
        ...
        if form.is_valid():
            ...
            first_test = form.cleaned_data['first_test']
            second_test = form.cleaned_data['second_test']
            session = form.cleaned_data['session_id']
            ...
            return redirect(reverse('staff_edit_result'))
        else:
            messages.error(request, 'Not Edited')
            return render(request, 'staff/edit_result.html', {'form': form})
...
def staff_student_result(request):
    ...
    if user.user_type != '2' or user.staff.current_position != 'teacher':
        ...
        if result:
            ...
            result_data = {'first_test': result.first_test, 'second_test': result.second_test}
